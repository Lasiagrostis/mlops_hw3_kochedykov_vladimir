...
Для развёртывания рабочего пайплайна необходимо
1. Клонировать репозиторий в рабочую директорию `git clone https://github.com/Lasiagrostis/mlops_hw3_kochedykov_vladimir.git`
2. Создать докер - образ, напрмер `docker build -t ci-cd-hw3:latest .`
3. Поднять два контейнера: blue и green версии:
```bash
